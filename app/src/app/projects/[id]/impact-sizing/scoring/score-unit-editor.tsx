    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_280px]">
      <div className="space-y-4">
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-4">
            {decisionTypes.map((dt) => (
              <label key={dt.id} className="rounded-md border border-slate-200 p-2 text-xs dark:border-slate-700">
                ...
                <p className="mt-0.5 line-clamp-2 text-[10px] text-slate-500">{dt.description[locale]}</p>
                <input
                  type="number"
                  min={0}
                  value={s.ddi[dt.id]}
            ...
            <label className="rounded-md border border-slate-200 p-2 text-xs dark:border-slate-700">
              ...
              <p className="mt-0.5 text-[10px] text-slate-500">{locale === "en" ? "Denominator" : "分母"}</p>
              <input
                type="number"
                min={1}
                value={s.totalSteps}
        ...
        <section className="rounded-md border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-3 flex items-baseline justify-end gap-1 border-t border-slate-100 pt-2 text-xs dark:border-slate-800">
            <span className="text-slate-500">{locale === "en" ? "High count:" : "高风险数:"}</span>
      ...
      <aside className="space-y-3">
        ...
        <div className="rounded-md border border-slate-200 bg-white p-4 text-xs dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-3 font-mono text-[10px] text-slate-400">Priority = RAS × (1 + 0.25 × DDI)</p>
        </div>
      </aside>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <section>
        ...
        {vmDimensions.map((d) => (
          <article key={d.id} className="mt-3 rounded-md border border-slate-200 p-2 text-xs dark:border-slate-800">
      ...
      <section>
        <h3 className="text-sm font-semibold">{locale === "en" ? "Risk Categories" : "风险类别"}</h3>
        <ul className="mt-2 space-y-1 text-xs">
          {riskCategories.map((cat) => (
