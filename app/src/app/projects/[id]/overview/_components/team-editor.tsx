...
const inputCls =
  "w-full rounded-md border border-zinc-300 bg-white px-2 py-1 text-sm dark:border-zinc-700 dark:bg-zinc-950";
  ...
  return (
    <section>
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">{t.project.team}</h3>
        {!editing && (
          <button
            type="button"
      ...
      {editing ? (
        <div className="mt-2 space-y-2 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          {members.map((member, index) => (
            <div key={index} className="flex items-center gap-2">
              ...
              <button
                ...
                className="shrink-0 rounded-md border border-rose-200 bg-white p-1.5 text-rose-600 hover:bg-rose-50 dark:border-rose-900/50 dark:bg-zinc-900 dark:text-rose-400 dark:hover:bg-rose-950/30"
              >
        ...
        <ul className="mt-2 space-y-1 rounded-xl border border-zinc-200 bg-white p-4 text-sm dark:border-zinc-800 dark:bg-zinc-900">
          {members.length === 0 ? (
            <li className="text-zinc-400">{en ? "No team members yet." : "尚无团队成员。"}</li>
          ) : (
            members.map((member, index) => (
              <li key={index} className={member.name ? "" : "text-zinc-400"}>
