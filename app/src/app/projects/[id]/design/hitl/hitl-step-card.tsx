...
const riskBadge: Record<RiskLevel, string> = {
  ...
  minimal: "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400",
};

const inputCls =
  "mt-1 w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-sm dark:border-slate-700 dark:bg-slate-950";
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-3 flex items-start gap-2 rounded-md border border-indigo-100 bg-indigo-50/60 p-2 dark:border-indigo-900/40 dark:bg-indigo-950/20">
        ...
        <button
          ...
          className="shrink-0 rounded-md border border-indigo-300 bg-white px-2 py-0.5 text-xs font-medium text-indigo-700 hover:bg-indigo-100 dark:border-indigo-800 dark:bg-slate-900 dark:text-indigo-300"
        >
          {en ? "Apply" : "采用"}
        </button>
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
        <label className="text-xs">
          <span className="block text-slate-500">{en ? "Checkpoint" : "介入时机"}</span>
          <select
        ...
        <label className="text-xs">
          <span className="block text-slate-500">{en ? "Escalate below confidence (%)" : "置信度阈值 (%)"}</span>
          <input
            type="number"
            min={0}
            max={100}
        ...
        <label className="text-xs">
          <span className="block text-slate-500">{en ? "Trigger" : "触发条件"}</span>
          <input
            value={hitl.trigger ?? ""}
        ...
        <label className="text-xs">
          <span className="block text-slate-500">{en ? "Reviewer" : "审核人"}</span>
          <input
            value={hitl.reviewer ?? ""}
        ...
        <label className="text-xs">
          <span className="block text-slate-500">{en ? "Escalation" : "升级路径"}</span>
          <input
