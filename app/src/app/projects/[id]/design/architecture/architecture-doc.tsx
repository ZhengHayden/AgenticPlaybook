...
export function ArchitectureDoc({ projectId, projectName, workflows, workflow }: ArchitectureDocProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <article className="space-y-6 rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
        <header>
        ...
        <section>
          <h2 className="border-b border-slate-200 pb-1 text-sm font-semibold uppercase tracking-wide text-slate-500 dark:border-slate-800">
          ...
          <textarea
            ...
            className="mt-2 w-full resize-y rounded-md border border-slate-200 bg-white p-3 text-sm dark:border-slate-800 dark:bg-slate-950"
            rows={4}
          />
        </section>
              ...
              return (
                <div key={step.id} className="rounded-md border border-slate-200 p-3 dark:border-slate-800">
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    ...
                    <strong className="text-slate-700 dark:text-slate-300">{archetype ? archetype[locale].name : "—"}</strong>
                    {"  ·  "}
                    {locale === "en" ? "Mode" : "模式"}:{" "}
                    <strong className="text-slate-700 dark:text-slate-300">{mode ? mode[locale].name : "—"}</strong>
                  </div>
                  <div className="mt-2 grid grid-cols-1 gap-1 text-xs md:grid-cols-3">
                    <div>
                      <span className="text-slate-500">{locale === "en" ? "Given" : "前置"}:</span> {step.inputs}
                    </div>
                    <div>
                      <span className="text-slate-500">{locale === "en" ? "When" : "触发"}:</span> {step.description}
                    </div>
                    <div>
                      <span className="text-slate-500">{locale === "en" ? "Then" : "结果"}:</span> {step.outputs}
                    </div>
                  </div>
                </div>
              );
